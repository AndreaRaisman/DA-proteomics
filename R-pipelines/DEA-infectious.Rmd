<<<<<<< HEAD:DEA-infectious.Rmd
---
title: "Differential Expression Analysis for infectious diseases"
...
## Differential Expression analysis for infectious diseases
...
infectious_temp <- clinical_info %>% filter(Class == 'Infection' & Disease != "Viral hepatitis related cirrhosis") 
infectious <- unique(infectious_temp$sample_id)
infectious <- colnames(temp_quant_data)[str_detect(colnames(temp_quant_data), paste(infectious, collapse = '|'))]

# Non-infectious samples samples 
non_infectious_temp <- clinical_info %>% filter(!(Class == 'Infection' & Disease != "Viral hepatitis related cirrhosis"))
non_infectious <- unique(non_infectious_temp$sample_id)
non_infectious <- colnames(temp_quant_data)[str_detect(colnames(temp_quant_data), paste(non_infectious, collapse = '|'))]
```
...
ttest_res <- apply(quant_data, 1, function(x, test_grp, ref_grp) {
  tidy(t.test(as.numeric(x[test_grp]),
              ...
              na.action = 'na.omit'))},
  test_grp = infectious, ref_grp = non_infectious)
...
writeLines(up_b, sep = '\n', con = 'Infectious_proteins_ttest_bonf.txt')
writeLines(down_b, sep = '\n', con = 'Non-infectious_proteins_ttest_bonf.txt')

# Save t-test results and quant data for DEPs
write.csv(ttest_res, 'Infectious_vs_non-infectious_ttest_bonf.csv')
ttest_deps <- rownames(ttest_res[ttest_res$bonferroni < 0.05,])
write.csv(quant_data[rownames(quant_data) %in% ttest_deps,],
          "quant_data_bonf_deps_infectious.csv")

x_b <- quant_data[quant_data$`...1` %in% ttest_deps,]
y_b <- as.data.frame(t(clinical_info))
#z <- bind_rows(x, y)
write.csv(y_b, 'bonf_test_infectious.csv')
```

```{r venn_diagram, fig.width=5, fig.height=5, echo=FALSE, warning=FALSE, message=FALSE}
# P-values
...
Venn_infectious <- ggvenn(list('No correction' = pval_deps,
            'Bonferroni' = bonf_deps,
            'FDR' = fdr_deps))

svg("Venn_infectious.svg", width = 5, height = 5)
Venn_infectious
dev.off()
...
**No p-value correction**: 168


**FDR (Benjamini-Hochberg) correction**: 165


---- Up in infectious diseases: 45


---- Down in infectious diseases: 120


**Bonferroni correction**: 136


---- Up in infectious diseases: 35


---- Down in infectious diseases: 101


# Volcano plots

The volcano plot is a visual tool to represent the results from the t-test. The p-value (-log10 transformed) is shown on the y-axis and the log2-fold change on the x-axis. Beyond applying multiple hypotheses testing corrections, applying a fold change cut-off can also reduce the FDR while tending to keep the true positives (Pascovici et al., 2016). For visualization purposes, the x-axis is going to be limited to (-8,8) fold-change. This excludes 17 peptides, whose names can be found in the list underneath the plot. 

```{r DEA_FDR, fig.width=10, fig.height=8, echo=FALSE, warning=FALSE, message=FALSE}
...
volcano_plot_infectious_FDR <- ggplot(ttest_res, aes(x = FC, y =  -log10(adj.p))) +
  ...
  labs(x = 'Fold change', y = '-log10(q-value)', title = 'Infectious vs non infectious diseases - FDR corrected t-test') +
  theme_bw() +
  xlim(-8,8)

svg("volcano_plot_infectious_FDR.svg", width = 12, height = 8)
volcano_plot_infectious_FDR
dev.off()
...
```{r DEA_Bonferroni, fig.width=10, fig.height=8, echo=FALSE, warning=FALSE, message=FALSE}
# Proteins to label in the plot
...
volcano_plot_infectious_bonferroni <- ggplot(ttest_res, aes(x = FC, y =  -log10(bonferroni))) +
  geom_vline(xintercept = c(0, -1, 1), linetype = 'dashed') +
  geom_hline(yintercept = -log10(0.05), linetype = 'dashed') +
  geom_point(color = 'grey30', alpha = 0.8) +
  geom_point(data = ttest_up, color = '#FF8089', alpha = 0.8) + 
  ...
  labs(x = 'Fold change', y = '-log10(q-value)', title = 'Infectious vs non infectious diseases - Bonferroni-corrected t-test') +
  theme_bw() +
  xlim(-8,8)

svg("volcano_plot_infectious_bonferroni.svg", width = 12, height = 8)
volcano_plot_infectious_bonferroni
dev.off()
...
```{r,  echo=FALSE, warning=FALSE, message=FALSE}
excluded_peptides_b <- ttest_res[ttest_res$FC > 8 | ttest_res$FC < -8,]
print(excluded_peptides_b[c("FC", "adj.p", "bonferroni")])
```

The clustering and principal component analyses can now be repeated based only on the differentially expressed proteins to visualize how well they can separate the samples belonging to infectious and non-infectious diseases (plus viral hepatitis-related cirrhosis) from each other. The first heatmap shows all the differentially expressed peptides that reached a significant FDR corrected value, while the second one only shows the peptide with the highest mean per protein. All in all, the heatmaps show basically same clustering tendencies as the ones from the initial analyses, confirming that the infectious diseases here considered seem to follow similar protein expression patterns. 

```{r echo=FALSE, warning=FALSE, message=FALSE}
infection_colors <- 
  c("Infectious" = "red3",
     "Non-infectious" = "lightblue3")
...
col_ann <- col_ann %>%
  mutate(Infectious = ifelse(Class == "Infection" & Disease != "Viral hepatitis related cirrhosis", "Infectious", "Non-infectious"))

col_ann_inf <- col_ann["Infectious"]
...
# Plot heatmap
all_sig_infectious_heatmap <- pheatmap(as.matrix(imputed_sig_data),
         ...
         annotation_col = col_ann_inf,
         show_colnames = F,
         border_color = NA,
         annotation_colors = list(Infectious = infection_colors),
...
svg("all_sig_infectious_heatmap.svg", width = 15, height = 8)
all_sig_infectious_heatmap
dev.off()
...
# Extracting sample, disease, and class from column names
col_ann_2 <- data.frame(colnames(highest_pep)) %>%
...
rownames(col_ann_2) <- col_ann_2$sample_id

# Remove the 'sample_id' column to avoid duplication
col_ann_2 <- col_ann_2[, -1]

col_ann_2 <- col_ann_2 %>%
  mutate(Infectious = ifelse(Class == "Infection" & Disease != "Viral hepatitis related cirrhosis", "Infectious", "Non-infectious"))

col_ann_2 <- col_ann_2["Infectious"]
...
# Plot heatmap
max_sig_pep_infectious_heatmap <- pheatmap(as.matrix(imputed_highest_pep),
         width = 24,  # Adjust the width of the plot
         height = 20,  # Adjust the height of the plot
         breaks = seq(-2, 2, length.out = 101),
         scale = 'row',
...
svg("max_sig_pep_infectious_heatmap.svg", width = 15, height = 8)
max_sig_pep_infectious_heatmap
dev.off()

max_sig_pep_infectious_heatmap
```

Regarding the PCA, the variance explained by each the first 3 PCs is very similar and the data points belonging to the infectious diseases occupy very similar positions to those seen in the PCA plot which considered all peptides. Without considering viral hepatitis-related cirrhosis as an infectious disease, the group formed by the diseases which can be considered as systemic infectious is quite well segretated into the top left corner, indicating that both PC1 and PC2 have a strong effect on these samples. In the case of PC3, which did not seem to provide much more information initially, the samples belonging to infectious diseases seem to show a tendency to go towards the top-right corner. All of the features in the PCs from this analysis were also in the same PC for the initial PCA. 
...
PCA_explained_var_infectious <- fviz_eig(pca_res_deps, main = 'Variance explained', addlabels = TRUE) + # barplot of variance explained
...
svg("PCA_explained_var_infectious.svg", width = 10, height = 7)
PCA_explained_var_infectious
dev.off()
...
id_disease_class_infection <- id_disease_class %>%
  mutate(Infectious = ifelse(Class == "Infection" & Disease != "Viral.hepatitis.related.cirrhosis", "Infectious", "Non-infectious"))

inf_factor <- as.factor(id_disease_class_infection$Infectious)

infection_colors <- 
  c("Infectious" = "red3",
     "Non-infectious" = "lightblue3")

disease <- as.factor(id_disease_class_infection$Disease)
...
# Plot PCA of samples
PCA_infectious_1_2 <- suppressWarnings(fviz_pca_ind(pca_res_deps,
             alpha.ind = 0,# Adjust transparency
             repel = TRUE,
             label = "none") + 
  geom_point(aes(colour = inf_factor, shape = disease), size = 1.2, alpha = 0.7, stroke = 1.5) + 
  scale_color_manual(values = infection_colors, guide = guide_legend(title = "Infectious")) +
...
svg("PCA_infectious_1_2.svg", width = 15, height = 8)
PCA_infectious_1_2
dev.off()
...
PC1_2_infectious_lodg <- fviz_pca_biplot(pca_res_deps, 
                repel = TRUE,
                axes = c(1,2),
                geom.ind = 'point',
                alpha.ind = 0.2,
...
svg("PC1_2_infectious_lodg.svg", width = 9, height = 6)
PC1_2_infectious_lodg
dev.off()
...
PC1_2_infectious_contrib <- fviz_contrib(pca_res_deps, choice = 'var', top = 20, axes = c(1,2))+ 
  theme(axis.text.x = element_text(angle = 0, hjust = 1)) +
  coord_flip() 

svg("PC1_2_infectious_contrib.svg", width = 8, height = 4)
PC1_2_infectious_contrib
dev.off()
...
# Plot PCA of samples
PCA_infectious_2_3 <- suppressWarnings(fviz_pca_ind(pca_res_deps,
             alpha.ind = 0,# Adjust transparency
             axes = c(2,3),
             repel = TRUE,
             label = "none") + 
...
svg("PCA_infectious_2_3.svg", width = 15, height = 8)
PCA_infectious_2_3
dev.off()
...
```{r dims2-3, fig.width=9, fig.height=6, echo=FALSE, warning=FALSE, message=FALSE}
# Plot PCA with variable contributions
PC2_3_infectious_lodg <- fviz_pca_biplot(pca_res_deps, 
                repel = TRUE,
                axes = c(2,3),
                geom.ind = 'point',
                alpha.ind = 0.2,
...
svg("PC2_3_infectious_lodg.svg", width = 9, height = 6)
PC2_3_infectious_lodg
dev.off()
...
```{r contributions_2-3, fig.width=8, fig.height=4, echo=FALSE, warning=FALSE, message=FALSE}
# Barplot of variable contribution to the two first principal components
PC2_3_infectious_contrib <- fviz_contrib(pca_res_deps, choice = 'var', top = 20, axes = c(2,3))+ 
  theme(axis.text.x = element_text(angle = 0, hjust = 1)) +
  coord_flip()

svg("PC2_3_infectious_contrib.svg", width = 8, height = 4)
PC2_3_infectious_contrib
dev.off()
...
output_file <- "top_sig_infectious_PCA_variables.xlsx"
...
# Run UMAP
umap_res <- umap(t(imputed_sig_data))
...
umap_coords$Hepatic <- col_ann$Infectious

# Plot results
UMAP_infectious <- ggplot(umap_coords, aes(x = V1, y = V2, alpha=0, label = "none")) +
  geom_point(aes(colour = Hepatic, shape = Disease), size = 1.5, alpha = 0.7, stroke = 1.5) +  # Adjust point size and alpha
  scale_color_manual(values = infection_colors, guide = guide_legend(title = "Infectious")) +
  scale_shape_manual(values = disease_shapes, guide =  guide_legend(title = "Disease")) +
  theme(legend.position = "bottom",
        legend.direction = "horizontal",
...
svg("UMAP_infectious.svg", width = 15, height = 8)
UMAP_infectious
dev.off()
...
# Run t-SNE
tsne_res <- Rtsne(t(imputed_sig_data), perplexity = 5)

# Extract coordinates
tsne_coords <- as.data.frame(tsne_res$Y)

# Add annotations
tsne_coords$Sample <- colnames(imputed_sig_data)
tsne_coords$Infectious <- col_ann$Infectious
tsne_coords$Disease <- col_ann$Disease


# PLot results
tSNE_infectious <- ggplot(tsne_coords, aes(x = V1, y = V2, color = Cell.line, label = Sample)) +
  geom_point(aes(colour = Infectious, shape = Disease), size = 1.5, alpha = 0.7, stroke = 1.5) +  # Adjust point size and alpha
  scale_color_manual(values = infection_colors, guide = guide_legend(title = "Class")) +
  scale_shape_manual(values = disease_shapes, guide =  guide_legend(title = "Disease")) +
  theme(legend.position = "bottom",
        legend.direction = "horizontal",
        legend.box.background = element_rect(colour = "black", linewidth = 0.5),
...
svg("tSNE_infectious.svg", width = 15, height = 8)
tSNE_infectious
dev.off()
...
ttest_deps <- rownames(ttest_res[ttest_res$adj.p < 0.05,])
write.csv(quant_data[rownames(quant_data) %in% ttest_deps,],
          "C:/Users/Uhlen Lab/Documents/Andrea/DA/Project Course 2/quant_data_deps.csv")
...
svg("all_sig_heatmap.svg", width = 15, height = 8)
all_sig_infectious_heatmap
dev.off()
...
svg("max_sig_pep_heatmap.svg", width = 15, height = 8)
max_sig_pep_infectious_heatmap
dev.off()
...
Storey, J. D., & Tibshirani, R. (2003). Statistical significance for genomewide studies. *Proceedings of the National Academy of Sciences, 100*(16), 9440-9445.
>>>>>>> 77d33f3f9e4068510419f6549665fbb5f45e87ed:R-pipelines/DEA-infectious.Rmd
