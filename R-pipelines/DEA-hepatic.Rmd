...
write.csv(quant_data[rownames(quant_data) %in% ttest_deps,],
          "C:/Users/Uhlen Lab/Documents/Andrea/DA/Project_Course_2/quant_data_deps_hepatic.csv")
...
write.csv(quant_data[rownames(quant_data) %in% ttest_deps,],
          "quant_data_bonf_hepatic_deps.csv")
...
Venn_hepatic<- ggvenn(list('No correction' = pval_deps,
            'Bonferroni' = bonf_deps,
            'FDR' = fdr_deps))

svg("Venn_hepatic.svg", width = 5, height = 5)
Venn_hepatic
dev.off()
...
volcano_plot_hepatic_FDR <- ggplot(ttest_res, aes(x = FC, y =  -log10(adj.p))) +
...
svg("volcano_plot_hepatic_FDR.svg", width = 12, height = 8)
volcano_plot_hepatic_FDR
dev.off()
...
volcano_plot_hepatic_bonferroni <- ggplot(ttest_res, aes(x = FC, y =  -log10(bonferroni))) +
  geom_vline(xintercept = c(0, -1, 1), linetype = 'dashed') +
  geom_hline(yintercept = -log10(0.05), linetype = 'dashed') +
  geom_point(color = 'grey30', alpha = 0.8) +
  geom_point(data = ttest_up, color = '#FF8089', alpha = 0.8) + 
...
svg("volcano_plot_hepatic_bonferroni.svg", width = 12, height = 8)
volcano_plot_hepatic_bonferroni
dev.off()
...
PCA_explained_var_hepatic <- fviz_eig(pca_res_deps, main = 'Variance explained', addlabels = TRUE) + # barplot of variance explained
...
svg("PCA_explained_var_hepatic.svg", width = 10, height = 7)
PCA_explained_var_hepatic
dev.off()
...
# Plot PCA of samples
PCA_hepatic_1_2 <- suppressWarnings(fviz_pca_ind(pca_res_deps,
...
svg("PCA_hepatic_1_2.svg", width = 15, height = 8)
PCA_hepatic_1_2
dev.off()
...
PC1_2_hepatic_lodg <- fviz_pca_biplot(pca_res_deps, 
                repel = TRUE,
                axes = c(1,2),
                geom.ind = 'point',
                alpha.ind = 0.2,
...
svg("PC1_2_hepatic_lodg.svg", width = 9, height = 6)
PC1_2_hepatic_lodg
dev.off()
...
PC1_2_hepatic_contrib <- fviz_contrib(pca_res_deps, choice = 'var', top = 20, axes = c(1,2))+ 
  theme(axis.text.x = element_text(angle = 0, hjust = 1)) +
  coord_flip()

svg("PC1_2_hepatic_contrib.svg", width = 8, height = 4)
PC1_2_hepatic_contrib
dev.off()
...
# Plot PCA of samples
PCA_hepatic_2_3 <- suppressWarnings(fviz_pca_ind(pca_res_deps,
             alpha.ind = 0,# Adjust transparency
             axes = c(2,3),
             repel = TRUE,
             label = "none") + 
...
svg("PCA_hepatic_2_3.svg", width = 15, height = 8)
PCA_hepatic_2_3
dev.off()
...
# Plot PCA with variable contributions
PC2_3_hepatic_lodg <- fviz_pca_biplot(pca_res_deps, 
                repel = TRUE,
                axes = c(2,3),
                geom.ind = 'point',
                alpha.ind = 0.2,
...
svg("PC2_3_hepatic_lodg.svg", width = 9, height = 6)
PC2_3_hepatic_lodg
dev.off()
...
# Barplot of variable contribution to the two first principal components
PC2_3_hepatic_contrib <- fviz_contrib(pca_res_deps, choice = 'var', top = 20, axes = c(2,3))+ 
  theme(axis.text.x = element_text(angle = 0, hjust = 1)) +
  coord_flip()     

svg("PC2_3_hepatic_contrib.svg", width = 8, height = 4)
PC2_3_hepatic_contrib
dev.off()
...
umap_coords$Hepatic <- col_ann$Hepatic

# Plot results
UMAP_hepatic <- ggplot(umap_coords, aes(x = V1, y = V2, alpha=0, label = "none")) +
...
svg("UMAP_hepatic.svg", width = 15, height = 8)
UMAP_hepatic
dev.off()
...
tsne_coords$Hepatic <- col_ann$Hepatic
tsne_coords$Disease <- col_ann$Disease


# PLot results
tSNE_hepatic <- ggplot(tsne_coords, aes(x = V1, y = V2, color = Cell.line, label = Sample)) +
  geom_point(aes(colour = Hepatic, shape = Disease), size = 1.5, alpha = 0.7, stroke = 1.5) +  # Adjust point size and alpha
  scale_color_manual(values = hepatic_colors, guide = guide_legend(title = "Class")) +
  scale_shape_manual(values = disease_shapes, guide =  guide_legend(title = "Disease")) +
  theme(legend.position = "bottom",
        legend.direction = "horizontal",
        legend.box.background = element_rect(colour = "black", linewidth = 0.5),
...
svg("tSNE_hepatic.svg", width = 15, height = 8)
tSNE_hepatic
dev.off()
...
Storey, J. D., & Tibshirani, R. (2003). Statistical significance for genomewide studies. *Proceedings of the National Academy of Sciences, 100*(16), 9440-9445.
>>>>>>> 77d33f3f9e4068510419f6549665fbb5f45e87ed:R-pipelines/DEA-hepatic.Rmd
